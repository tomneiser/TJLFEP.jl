...
!  print *, 'In TGLFEP_ky (3) factor_in=', factor_in
  ...
  if (l_debug_write) print *, 'Step 1 lkeep ', lkeep(:) 
...
!    QL_flux_ratio(n) = DEP(n)/chi_i_cond(n)
  ...
  if (l_debug_write) print *, 'Step 1.5 lkeep(0)', lkeep(1), ltearing(1), l_i_pinch(1), l_e_pinch(1), l_th_pinch(1), &
          l_EP_pinch(1), l_QL_ratio(1), l_theta_sq(1)
...
!    if (reject_max_outer_panel_flag .eq. 1) lkeep(n) = lkeep(n) .and. .not. l_max_outer_panel(n)
    if (l_debug_write) print *, 'Loop ', n, 'Step 1.6 lkeep(0)', lkeep(1), ltearing(1), l_i_pinch(1), l_e_pinch(1), l_th_pinch(1), &
          l_EP_pinch(1), l_QL_ratio(1), l_theta_sq(1)
    lkeep(n) = lkeep(n) .and. .not. l_QL_ratio(n)
    if (l_debug_write) print *, 'Loop ', n, 'Step 1.7 lkeep(0)', lkeep(1), ltearing(1), l_i_pinch(1), l_e_pinch(1), l_th_pinch(1), &
          l_EP_pinch(1), l_QL_ratio(1), l_theta_sq(1)
    lkeep(n) = lkeep(n) .and. .not. l_theta_sq(n)
    if (l_debug_write) print *, 'Loop ', n, 'Step 1.8 lkeep(0)', lkeep(1), ltearing(1), l_i_pinch(1), l_e_pinch(1), l_th_pinch(1), &
          l_EP_pinch(1), l_QL_ratio(1), l_theta_sq(1)    
  enddo

  if (l_debug_write) print *, 'Step 2 lkeep(0)', lkeep(1), ltearing(1), l_i_pinch(1), l_e_pinch(1), l_th_pinch(1), &
          l_EP_pinch(1), l_QL_ratio(1), l_theta_sq(1)
!  if (l_debug_write) then
...
!  endif
  ...
  if (l_wavefunction_out .eq. 1) then
    ...
    write (22,*) 'Mode kept T/F for each mode:  ', lkeep(:)
    write (22,*) 'Theta      Re(phi)        Im(phi)        Re(B_per)        Im(B_per)'
...
!      enddo
  ...
  if (l_debug_write) print *, 'Step 3 lkeep(0)', lkeep(1), ltearing(1), l_i_pinch(1), l_e_pinch(1), l_th_pinch(1), &
          l_EP_pinch(1), l_QL_ratio(1), l_theta_sq(1)
