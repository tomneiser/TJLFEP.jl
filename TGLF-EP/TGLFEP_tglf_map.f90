...
!  print *, 'In tglf_map (1) factor_in=', factor_in
  ...
  if (scan_method .eq. 1) tglf_as_in(is) = as(ir,is)*factor_in   !  if logic EMB 2/2023
...
!      if (id_2==0) print *, 'species:', i, '   grid:', ir, '  rlns:', tglf_rlns_in(i), '  rlts:', tglf_rlts_in(i)
  ...
  if(mode_in .eq. 4) then ! Preserve all drive
    do i = 1,ns  ! EMB changed 2 to ns and added "if" logic 6/17/2022
      tglf_rlns_in(i) = rlns(ir,i)
      tglf_rlts_in(i) = rlts(ir,i)
    enddo
...
!    if (id_2==0) print *, 'grid:', ir, '    scan factor:', scan_factor
  ...
  if(geometry_flag .eq. 1) then
    ...
    do i = 1,ns
      ...
      if (i .eq. is) then
        sum2 = sum2 + tglf_as_in(i)*tglf_taus_in(i)*(tglf_rlns_in(i)+tglf_rlts_in(i))
      else
        sum2 = sum2 + as(ir,i)*taus(ir,i)*(rlns(ir,i)+rlts(ir,i))
      endif
    enddo
    select case(pprime_method)    ! pprime_method added 10-9-2024, EMB
      case(1)
        tglf_p_prime_loc_in = p_prime(ir)               ! Fixed beta_* stabilization
      case(2)
        tglf_p_prime_loc_in = p_prime(ir)*sum0/sum1     ! beta_* stabilization matches kinetic drives
      case(3)
        tglf_p_prime_loc_in = p_prime(ir)*sum2/sum1     ! beta_* stabilization preserves thermal piece, varies with EP
      case default
         print *, 'Invalid PPRIME_METHOD, reverting to fixed p_prime'
         tglf_p_prime_loc_in = p_prime(ir)
    end select
...
!  freq_AE_upper = freq_cutoff*abs(omega_TAE(ir)) !freq_cutoff*omega_TAE
  freq_AE_upper = -abs(omega_GAM(ir))
  if (rotational_suppression_flag .eq. 1) then
    gamma_thresh_max = abs(gamma_p(ir))*2.0*(min(1.-rmin(ir),rmin(ir))/rmaj(ir))
    gamma_thresh = 0.15*abs(gamma_E(ir)/shear(ir))   ! Bass PoP 2017 flow-shear suppression of AEs
    gamma_thresh = min(gamma_thresh,gamma_thresh_max)
  else
    gamma_thresh = 1.0E-7
  endif
...
subroutine read_input_profile
  ...
  if(iexist) then
    ...
    do i = 1,nr
      ...
      omega_GAM(i) = (1./rmaj(i))*(1+taus(i,2))**0.5/(1.+1./(2.*q(i)))
    enddo
