...
# (see verify/batch_debug_nb6_fortran_scan20_10n.sh). Side-by-side:
#
#   Fortran TGLF-EP (CPU, 10 nodes)          TJLFEP (Julia, GPU, 5 nodes)
#   -------------------------------          ----------------------------
#   DRIVER=$TGLFEP_DIR/TGLFEP_driver         DRIVER=$TJLFEP_ROOT/build/common/run_gacode_scan20_mps_task.jl
...
#
# Usage:  edit the CONFIG paths below, then (from build/):  sbatch run/submit_tjlfep_gpu_5N_example.sh
...
# 2) Prebuilt GPU sysimage (file-only). Build it ONCE with sysimage/batch_build_gpu_sysimage_generic.sh,
...
DRIVER="${TJLFEP_ROOT}/build/common/run_gacode_scan20_mps_task.jl"
WRAPPER="${TJLFEP_ROOT}/build/common/mps-scan-wrapper.sh"
MERGE="${TJLFEP_ROOT}/build/common/merge_gacode_scan20_array.jl"
