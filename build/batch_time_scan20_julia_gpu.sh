#!/bin/bash -l
# Timing: Julia GPU SCAN_N=20, 5 nodes, 20 srun tasks + CPU merge (gacode path).
#SBATCH -A m3739_g
#SBATCH -q premium
#SBATCH -N 5
#SBATCH -n 20
...
export OUT_DIR="${TJLFEP_ROOT}/build/gacode_scan20_time_${SLURM_JOB_ID}_tasks"

cd "${TJLFEP_ROOT}/build"

JOB_T0=$(date +%s.%N)
echo "TIMING_START backend=julia device=gpu path=gacode nodes=${SLURM_NNODES:-5} tasks=${SLURM_NTASKS:-20} SCAN_N=20 N_BASIS=6"
...
echo "TIMING_RESULT backend=julia device=gpu phase=scan seconds=${SCAN_S} SCAN_N=20 N_BASIS=6 nodes=${SLURM_NNODES:-5} tasks=${SLURM_NTASKS:-20}"
...
echo "TIMING_RESULT backend=julia device=gpu phase=merge seconds=${MERGE_S} SCAN_N=20 N_BASIS=6"

JOB_T1=$(date +%s.%N)
TOTAL_S=$(python3 -c "print(f'{float(\"${JOB_T1}\") - float(\"${JOB_T0}\"): .3f}')")
echo "TIMING_RESULT backend=julia device=gpu phase=total_job seconds=${TOTAL_S} SCAN_N=20 N_BASIS=6 nodes=${SLURM_NNODES:-5} tasks=${SLURM_NTASKS:-20}"
