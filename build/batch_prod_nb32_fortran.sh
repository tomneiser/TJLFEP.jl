#!/bin/bash -l
# Production Fortran validation N_BASIS=32 (writable input.TGLFEP).
#SBATCH -A m3739
#SBATCH -q premium
#SBATCH -N 10
#SBATCH -t 01:00:00
#SBATCH -C cpu
#SBATCH -J TGLFEP_prod32
#SBATCH -o prod_nb32_fortran_%j.out
#SBATCH -e prod_nb32_fortran_%j.err
...
PROD_DIR="${TJLFEP_ROOT}/build/debug_prod"
DRIVER="${TGLFEP_DIR}/TGLFEP_driver"
RUN_DIR="${TJLFEP_ROOT}/build/fortran_runs/prod_nb32_${SLURM_JOB_ID:-local}"
...
mkdir -p "${RUN_DIR}"
cd "${RUN_DIR}"
ln -sf "${PROD_DIR}/input.TGLFEP" input.TGLFEP
ln -sf "${CASE_DIR}/input.gacode" input.gacode

srun -n "${SLURM_NTASKS:-1280}" "${DRIVER}"
